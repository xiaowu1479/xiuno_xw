...
if(empty($action)) {
	...
	$pdo_mysql_support = extension_loaded('pdo_mysql');
		...
		$adminpass_confirm = param('adminpass_confirm');
		...
		empty($adminpass_confirm) AND message('adminpass_confirm', lang('adminpass_confirm_is_empty'));
		$adminpass !== $adminpass_confirm AND message('adminpass', lang('adminpass_not_match'));
		empty($adminemail) AND message('adminemail', lang('adminuser_is_empty'));
		...
		// 管理员密码
		$password = user_password_hash($adminpass);
		$update = array('username'=>$adminuser, 'email'=>$adminemail, 'password'=>$password, 'create_date'=>$time, 'create_ip'=>$longip);
