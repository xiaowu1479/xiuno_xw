...
CREATE TABLE `bbs_user` (
  ...
  `password` char(60) NOT NULL DEFAULT '' COMMENT '密码',			# bcrypt 60 chars
  `password_sms` char(16) NOT NULL DEFAULT '' COMMENT '密码',	# 预留，手机发送的 sms 验证码
  salt char(16) NOT NULL DEFAULT '' COMMENT '密码混杂',			# 向后兼容保留
...
DROP TABLE IF EXISTS bbs_tag_cate;
DROP TABLE IF EXISTS bbs_tag_thread;
DROP TABLE IF EXISTS bbs_post_like;
DROP TABLE IF EXISTS bbs_haya_favorite;
DROP TABLE IF EXISTS bbs_sg_sign;
DROP TABLE IF EXISTS bbs_sg_sign_set;
DROP TABLE IF EXISTS bbs_thread_search;
