...
!db_sql_find_one("SHOW COLUMNS FROM {$tablepre}thread LIKE 'tagids'") AND db_exec("ALTER TABLE {$tablepre}thread ADD COLUMN tagids char(32) NOT NULL DEFAULT ''");

// 缓存的时间，用来和 setting('tag_update_time') 对比
!db_sql_find_one("SHOW COLUMNS FROM {$tablepre}thread LIKE 'tagids_time'") AND db_exec("ALTER TABLE {$tablepre}thread ADD COLUMN tagids_time int(11) unsigned NOT NULL DEFAULT '0'");
