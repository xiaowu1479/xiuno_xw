...
$sql = "
CREATE TABLE IF NOT EXISTS {$tablepre}haya_favorite (
...
!db_sql_find_one("SHOW COLUMNS FROM {$tablepre}thread LIKE 'favorites'") AND db_exec("ALTER TABLE {$tablepre}thread ADD COLUMN favorites int(11) NULL DEFAULT '0' COMMENT '收藏数'");
!db_sql_find_one("SHOW COLUMNS FROM {$tablepre}user LIKE 'favorites'") AND db_exec("ALTER TABLE {$tablepre}user ADD COLUMN favorites int(11) NULL DEFAULT '0' COMMENT '收藏数'");
