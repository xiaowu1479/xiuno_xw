...
/**
 * @var string $keyword_decode 解码并过滤过的搜索关键词
 */
$keyword_decode = search_keyword_safe(xn_urldecode($keyword));

/**
 * @var array $keyword_arr 搜索关键词数组
 */
$keyword_arr = explode(' ', $keyword_decode);

/**
 * @var array $threadlist 帖子列表数组
 * @var array $postlist 回帖列表数组
 */
$threadlist = $postlist = array();

/**
 * @var string $pagination 翻页HTML
 */
$pagination = '';

/**
 * @var string $active 当前激活状态
 */
$active = '';

/**
 * @var array $search_conf 搜索插件设置
 */
...
function search_thread_by_like($keyword_decode, $start, $pagesize) {
	// 限制递归调用次数
	static $call_count = 0;
	if($call_count++ > 5) return array();
	$arrlist = db_sql_find("SELECT * FROM `bbs_thread` WHERE $keyword_decode ORDER BY `create_date` DESC LIMIT $start, $pagesize;");
	$tids = arrlist_values($arrlist, 'tid');
	$threadlist = thread_find_by_tids($tids);
	$threadlist = arrlist_multisort($threadlist, 'tid', FALSE);	
	global $forumlist, $gid;
	...
	if($count_before - $count_after > $less_number) {
		$pagesize *= 2;
		$threadlist = search_thread_by_like($keyword_decode, $start, $pagesize);
	}
	return $threadlist;
}

function search_post_by_like($keyword_decode, $start, $pagesize2, &$nextpage, &$page) {
	global $pagesize;
	
	// 限制递归调用次数
	static $call_count = 0;
	if($call_count++ > 5) return array();
	$arrlist = db_sql_find("SELECT * FROM `bbs_post` WHERE $keyword_decode ORDER BY `create_date` DESC LIMIT $start, $pagesize2;");
	$nextpage =  count($arrlist) == $pagesize2 ? $page + 1 : 0;
	$pids = arrlist_values($arrlist, 'pid');
	$postlist = post_find_by_pids($pids);
	...
	if($count_before - $count_after > $less_number) {
		$pagesize2 += $pagesize;
		$page++;
		$postlist = search_post_by_like($keyword_decode, $start, $page, $pagesize2, $nextpage, $page);
	}
...
if($keyword) {
	...
	if($search_type == 'fulltext') {
		...
		if($range == 1) {
			...
			$total = $arr['num'];
			$pagination = pagination(url("search-$keyword-$range-$exact-{page}"), $total, $page, $pagesize);
		...
		if($range == 1) {
			if($exact == 1) {
				foreach($keyword_arr as $key => $cond_val){
					$cond_sql .= "subject LIKE '%$cond_val%'";
					if($key != count($keyword_arr) - 1)$cond_sql .= ' AND ';
				}
			}else{
				foreach($keyword_arr as $key => $cond_val){
					$cond_sql .= "subject LIKE '%$cond_val%'";
					if($key != count($keyword_arr) - 1)$cond_sql .= ' OR ';
				}
			}
			$arr = db_sql_find_one("SELECT COUNT(*) AS num FROM bbs_thread WHERE $cond_sql");
			$total = $arr['num'];
			$pagination = pagination(url("search-$keyword-$range-$exact-{page}"), $total, $page, $pagesize);
			$start = ($page - 1) * $pagesize;
			$threadlist = search_thread_by_like($cond_sql, $start, $pagesize);
			foreach($threadlist as &$thread) {
				$thread['subject'] = search_keyword_highlight($thread['subject'], $keyword_arr);
			}
		} else if($range == 0) {
			if($exact == 1) {
				foreach($keyword_arr as $key => $cond_val){
					$cond_sql .= "message LIKE '%$cond_val%'";
					if($key != count($keyword_arr) - 1)$cond_sql .= ' AND ';
				}
			}else{
				foreach($keyword_arr as $key => $cond_val){
			...
			$arr = db_sql_find_one("SELECT COUNT(*) AS num FROM bbs_post WHERE $cond_sql");
			$total = $arr['num'];
			$pagination = pagination(url("search-$keyword-$range-$exact-{page}"), $total, $page, $pagesize);
			$start = ($page - 1) * $pagesize;
			$postlist = search_post_by_like($cond_sql, $start, $pagesize, $nextpage, $page);
			foreach($postlist as &$post) {
				$post['message_fmt'] = search_message_format($post['message_fmt']);
				$post['message_fmt'] = search_keyword_highlight($post['message_fmt'], $keyword_arr);
				$post['filelist'] = array();
...
if($ajax) {
	...
	include _include(APP_PATH.'plugin/fox_search/fox/theme/search.php');
}
?>