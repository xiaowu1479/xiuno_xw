...
/*
 * 奇狐搜索插件LIKE强化版 搜索设置
 * admin/plugin-setting-fox_search.htm
*/
...
if($action == 'set') {
    if($method == 'GET') {
        ...
        $input['style'] = form_radio('style', array(0=>'开启', 1=>'关闭'), $search_conf['style']);
        include _include(APP_PATH.'plugin/fox_search/fox/theme/setting.php');
    } else {
        $search_conf['type'] = param('type');
        $search_conf['range'] = param('range');
        $search_conf['site_url'] = param('site_url');
        $search_conf['style'] = param('style');
        kv_set('search_conf', $search_conf);
        message(0, '修改成功');
    }
// 切词、索引，跳转的方式开始执行任务，一次执行 10 条，如果超时，则重新开始任务。
    ...
    $input = array();
    $subject_start = intval(kv_get('fox_search_subject_start'));
    $post_start = intval(kv_get('fox_search_post_start'));
    ...
    include _include(APP_PATH.'plugin/fox_search/fox/theme/setting_cn_encode.php');
    ...
    if($range == 1) {
        ...
        if(empty($tidlist)) {
            $start = $threads;
            kv_set('fox_search_subject_start', $start);
            message(0, jump('重建索引完毕。', url('plugin-setting-fox_search-cn_encode')));
        } else {
        ...
        $url = url("plugin-setting-fox_search-rebuild-$range-$start");
    ...
    // 帖子
        ...
        if(empty($pidlist)) {
            $start = $posts;
            kv_set('fox_search_post_start', $start);
            message(0, jump('重建索引完毕。', url('plugin-setting-fox_search-cn_encode')));
        } else {
