...
function post_read($pid, $thread = NULL) {
	// hook model_post_read_start.php
	$post = post__read($pid);
	post_format($post, $thread);
...
// 此处有缓存，是否有必要？
function post_find_by_tid($tid, $page = 1, $pagesize = 50, $thread = NULL) {
	global $conf;
...
function post_format(&$post, $thread = NULL) {
	...
	$thread = $thread ? $thread : thread_read_cache($post['tid']);
