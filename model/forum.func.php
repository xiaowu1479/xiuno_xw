...
function forum_format(&$forum) {
	...
	$forum['icon_url'] = $forum['icon'] ? $conf['upload_url']."forum/$forum[fid].png?$forum[icon]" : 'view/img/forum.png';
...
// $show_all = true: 不隐藏，标记 forbidden=1
function forum_list_access_filter($forumlist, $gid, $allow = 'allowread', $show_all = false) {
	global $conf, $grouplist;
	if(empty($forumlist)) return array();
	if($gid == 1) {
		// 管理员看到所有版块
		if($show_all) {
			foreach($forumlist as $fid=>&$_forum) {
				$_forum['forbidden'] = 0;
				unset($_forum['accesslist']);
			}
			unset($_forum);
		}
		return $forumlist;
	...
	foreach($forumlist_filter as $fid=>$forum) {
		$forbidden = (empty($forum['accesson']) && empty($group[$allow]) || !empty($forum['accesson']) && empty($forum['accesslist'][$gid][$allow]));
		if($show_all) {
			$forumlist_filter[$fid]['forbidden'] = $forbidden ? 1 : 0;
		} elseif($forbidden) {
