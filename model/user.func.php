...
function user_create($arr) {
	...
	if (!empty($arr['password'])) {
		$arr['password'] = user_password_hash($arr['password']);
	}
	$r = user__create($arr);
...
function user_token_get_do() {
	...
	// 绑定 IP，防止 token 被盗用后异地登录（可在后台关闭，移动网络下 IP 会变化）
	if(!empty($conf['token_bind_ip']) && $ip != $_ip) return FALSE;
	// token 过期时间，默认 7 天
	$token_expire = empty($conf['token_expire']) ? 604800 : $conf['token_expire'];
	if($time - $_time > $token_expire) return FALSE;
	// 检查密码是否被修改。
	if($time - $_time > 1800) {
		$user = user_read($_uid);
...
// 密码哈希 — 自动选择 bcrypt，兼容旧 MD5+Salt
function user_password_hash($pw) {
    return password_hash($pw, PASSWORD_BCRYPT);
}

function user_password_verify($pw, $hash_or_user) {
    if (is_array($hash_or_user)) {
        $hash = $hash_or_user['password'];
        if (strlen($hash) === 32 && ctype_xdigit($hash) && !empty($hash_or_user['salt'])) {
            return md5($pw . $hash_or_user['salt']) === $hash;
        }
    } else {
        $hash = $hash_or_user;
...
function csrf_check() {
    $token = param('csrf_token', '');
    if (empty($token) || $token !== csrf_token()) {
        message(-1, 'CSRF token validation failed');
    }
}
