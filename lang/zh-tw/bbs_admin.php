<?php

return array(
	'user'=>'用戶',
	'forum'=>'版塊',
	'plugin'=>'插件',
	'other'=>'其他',
	'buy'=>'購買',
	
	'user_admin'=>'用戶管理',
	'group_admin'=>'用戶組管理',
	'forum_admin'=>'版塊管理',
	'post_admin'=>'帖子管理',
	'thread_admin'=>'主題管理',
	'plugin_admin'=>'插件管理',
	'other_admin'=>'其他管理',
	
	'admin_index_page'=>'後臺',
	'front_index_page'=>'前臺',
	
	'online_hold_time'=>'在線保持時間（秒）',
	'online_hold_time_tips'=>'在線人數統計的時間範圍：在此時間內活躍過的訪客計入在線，預設 3600（1小時），小站建議 600（10分鐘）',
	'admin_site_setting'=>'站點設置',
	'admin_setting_base'=>'基本設置',
	'admin_setting_smtp'=>'SMTP 設置',
	
	// 检测更新 (XIUNO XW)
	'admin_other_update'=>'檢測更新',
	'admin_update_current_version'=>'當前版本',
	'admin_update_latest_version'=>'最新版本',
	'admin_update_check_now'=>'立即檢查',
	'admin_update_latest'=>'已是最新版本',
	'admin_update_found'=>'發現新版本',
	'admin_update_release_date'=>'發佈時間',
	'admin_update_notes'=>'更新說明',
	'admin_update_error'=>'無法連接更新伺服器，請檢查網路後重試',
	'admin_update_download'=>'下載原始碼',
	'admin_update_github'=>'GitHub 頁面',
	'admin_update_auto'=>'一鍵自動更新',
	'admin_update_confirm'=>'確定要自動更新到最新版嗎？系統會自動備份資料並覆蓋主程式檔案，期間請勿關閉頁面。',
	'admin_update_locked'=>'更新任務正在進行中，請稍後再試',
	'admin_update_no_download'=>'未取得可用的更新套件下載位址',
	'admin_update_download_failed'=>'更新套件下載失敗，請檢查伺服器網路後重試',
	'admin_update_bad_package'=>'更新套件格式不正確（僅支援 zip / tar.gz）',
	'admin_update_unzip_failed'=>'更新套件解壓失敗，請重試',
	'admin_update_success'=>'自動更新完成，已升級到版本 {version}',
	'admin_update_backup_done'=>'已自動備份到 {dir}，可在下方「回滾備份」中一鍵回滾',
	'admin_update_backup_title'=>'回滾備份',
	'admin_update_backup_tip'=>'每次自動更新 / 回滾前會自動備份主程式到 backup/ 目錄，僅保留最近 2 份，可一鍵回滾到任意一份。',
	'admin_update_backup_empty'=>'暫無備份',
	'admin_update_backup_time'=>'備份時間',
	'admin_update_backup_version'=>'版本',
	'admin_update_rollback'=>'回滾',
	'admin_update_rollback_confirm'=>'確定要回滾到該備份嗎？主程式檔案將被覆蓋，上傳/配置/日誌不會受影響，期間請勿關閉頁面。',
	'admin_update_rollback_success'=>'回滾成功，已恢復到備份 {dir}',
	'admin_update_backup_delete'=>'刪除',
	'admin_update_backup_delete_confirm'=>'確定刪除該備份嗎？此操作不可恢復！',
	'admin_update_backup_deleted'=>'備份已刪除',
	'admin_update_backup_invalid'=>'備份目錄名不合法',
	'admin_update_backup_not_exists'=>'備份不存在',
	'admin_other_cache'=>'清理緩存',
	'admin_clear_tmp'=>'清理臨時目錄',
	'admin_clear_cache'=>'清理緩存',
	'admin_clear_successfully'=>'清理成功',
	
	'admin_user_list'=>'用戶列表',
	'admin_thread_batch'=>'主題批量管理',
	'admin_user_group'=>'用戶組',
	'admin_user_create'=>'創建用戶',
	'admin_plugin_local_list'=>'本地插件',
	'admin_plugin_official_list'=>'官方插件',
	'admin_plugin_official_free_list'=>'免費插件',
	'admin_plugin_official_fee_list'=>'收費插件',
	
	'admin_token_error'=>'管理令牌錯誤，可能因為您的網絡環境不穩定，可以嘗試取消後臺綁定 IP，配置 conf.php，admin_bind_ip => 0 ',
	'admin_token_expiry'=>'管理登錄令牌失效，請重新登錄',
	'forum_edit'=>'版塊編輯',
	'user_edit'=>'用戶編輯',
	'edit_sucessfully'=>'編輯成功',
	'item_not_exists'=>'{item} 不存在',
	'item_not_moderator'=>'{item} 不是版主',
	'group_not_exists'=>'用戶組不存在',
	
	'admin_login'=>'管理登錄',
	'save_conf_failed'=>'保存數據到配置文件 {file} 失敗，請檢查文件的可寫權限',
	'user_already_exists'=>'用戶已經存在',
	'email_already_exists'=>'郵箱已經存在',
	'uid_not_exists'=>'指定的 UID 不存在',
	'data_not_changed'=>'沒有數據變動',
	'admin_cant_be_deleted'=>'不能直接刪除管理員，請先編輯為普通用戶組',
	
	// 首頁
	'admin_index'=>'後臺首頁',
	'site_stat_info'=>'站點統計信息',
	'disk_free_space'=>'磁盤剩余空間',
	'server_info'=>'服務器信息',
	'os'=>'操作系統',
	'post_max_size'=>'最大 POST 數據大小',
	'upload_max_filesize'=>'最大文件上傳大小',
	'allow_url_fopen'=>'允許開啟遠程 URL',
	'safe_mode'=>'安全模式（safe_mode）',
	'max_execution_time'=>'最長執行時間',
	'memory_limit'=>'內存上限',
	'client_ip'=>'客戶端 IP',
	'server_ip'=>'服務端 IP',
	'dev_team_info'=>'開發團隊信息',
	
	'for_safe_input_password_again'=>'為了您的安全，請再次輸入賬戶密碼',
	
	// 設置
	'sitename'=>'站點名稱',
	'sitebrief'=>'站點介紹',
	'sitebrief_tips'=>'註：支持 HTML 標簽，換行請使用 &lt;br&gt;',
	'runlevel'=>'站點訪問限制',
	'user_create_on'=>'开启用户注册',
	'captcha'=>'圖形驗證碼',
	'captcha_login_on'=>'登入驗證碼',
	'captcha_reg_on'=>'註冊驗證碼',
	'captcha_post_on'=>'發文驗證碼',
	'captcha_tips'=>'圖形驗證碼設定：登入/註冊/發文時需輸入圖片中的字元',
	'user_create_email_on'=>'開啟註冊郵箱驗證',
	'user_resetpw_on'=>'開啟找回密碼',
	'lang'=>'語言',
	'nav_2_on'=>'是否开启二级导航',
	'nav_2_forum_list_pc_on'=>'是否在二级导航下显示版块列表（PC）',
	'nav_2_forum_list_mobile_on'=>'是否在二级导航下显示版块列表（手机）',
	'database'=>'語言',
	'host'=>'主機',
	'port'=>'端口',
	'account'=>'賬號',
	'smtp_host'=>'SMTP 主機',
	
	// 版塊
	'forum_threads'=>'主題數',
	'forum_list'=>'版塊列表',
	'forum_id'=>'版塊 ID',
	'forum_icon'=>'圖標',
	'forum_name'=>'名稱',
	'forum_rank'=>'排序',
	'forum_view'=>'查看',
	'forum_edit'=>'編輯',
	'forum_delete'=>'刪除',
	'forum_brief'=>'簡介',
	'forum_announcement'=>'公告',
	'moderator'=>'版主',
	'add_new_line'=>'增加壹行',
	'forum_edit_tip'=>'請謹慎編輯版塊，壹旦確定後不要輕易變動，否則可能會導致數據關聯錯誤，壹般在正式運營時就不要再變動。',
	'forum_cant_delete_system_reserved'=>'不能刪除系統保留的版塊。',
	'forum_moduid_format_tips'=>'最多允許10個，逗號隔開，如：Jack,Lisa,Mike',
	'user_privilege'=>'用戶權限',
	'allow_view'=>'允許看帖',
	'allow_thread'=>'發主題',
	'allow_post'=>'回貼',
	'allow_upload'=>'上傳',
	'allow_download'=>'下載',
	'forum_delete_thread_before_delete_forum'=>'请先通过批量主题管理删除版块主题。',
	'forum_please_delete_sub_forum'=>'请删除子版块。',
	'forum_delete_successfully'=>'删除成功。',
	
	// 導航選單 (XIUNO XW)
	'nav_admin'=>'導航選單',
	'nav_name'=>'選單名稱',
	'nav_url'=>'連結網址',
	'nav_target'=>'新視窗',
	'nav_rank'=>'排序',
	'nav_tip'=>'選單項目顯示在前台頂部導航（首頁之後、版塊之前）。連結可以是站內位址（如 ?forum-2.htm）或外部網址（如 https://example.com）。',
	'nav_quick_add'=>'快速新增版塊',
	'nav_quick_add_select'=>'選擇版塊...',
	'nav_already_added'=>'該版塊已在導航選單中。',
	
	'thread_queue_not_exists'=>'隊列不存在',
	'search_condition'=>'搜索條件',
	'start_date'=>'開始時間',
	'end_date'=>'結束時間',
	'searching'=>'正在搜索',
	'search_complete'=>'搜索完成',
	'operating'=>'正在操作',
	'operator_complete'=>'操作完成',
	'click_to_view'=>'點擊查看',
	'thread_userip'=>'發帖 IP',
	'thread_search_result'=>'結果：{n} 條',
	
	// 用戶
	
	'please_check_delete_user'=>'請勾選您要刪除的用戶',
	'user_delete_confirm'=>'確定刪除用戶？',
	'user_admin_cant_be_deleted'=>'不允許刪除管理員用戶，如果確實要刪除，請先調整用戶組!',
	'search_type'=>'搜索類型',
	'user_privileges'=>'用戶權限',
	'author'=>'作者',
	
	// 用戶組
	'group_list'=>'用戶組列表',
	'group_edit'=>'用戶組編輯',
	'group_id'=>'用戶組 ID',
	'group_name'=>'用戶組名',
	'group_credits_from'=>'起始積分',
	'group_credits_to'=>'結束積分',
	'group_id'=>'用戶組ID',	'group_edit_tips'=>'請謹慎編輯用戶組，壹旦確定後不要輕易變動，否則可能會導致用戶關聯錯誤，壹般在正式運營時就不要再變動。',
	'admin_privilege'=>'管理權限',
	'top'=>'置頂',
	'ban_user'=>'禁止用戶',
	'delete_user'=>'刪除用戶',
	'view_user_info'=>'查看用戶信息',
	
	// 插件
	'plugin_dir'=>'插件目錄名',
	'plugin_bbs_version'=>'要求 BBS 最低版本',
	'price'=>'價格',
	'installs'=>'安裝次數',
	'plugin_user_stars_fmt'=>'用戶評級',
	'plugin_sells'=>'銷售次數',
	'plugin_is_cert'=>'通過官方認證',
	'local_plugin'=>'本地插件',
	'official_plugin'=>'官方插件',
	'pugin_cate_0'=>'所有插件',
	'pugin_cate_1'=>'風格模板',
	'pugin_cate_2'=>'小型插件',
	'pugin_cate_3'=>'大型插件',
	'pugin_cate_4'=>'接口整合',
	'pugin_cate_99'=>'未分類',
	'plugin_detail'=>'插件詳情',
	'plugin_brief_url'=>'插件介紹網址',
	'plugin_not_exists'=>'插件不存在',
	'plugin_versio_not_match'=>'此插件依賴的 XIUNO XW 最低版本為 {bbs_version} ，您當前的版本：{version}',
	'plugin_download_sucessfully'=>'下載插件 ({dir}) 成功，請點擊進行安裝',
	'plugin_install_sucessfully'=>'安裝插件 ( {name} ) 成功',
	'plugin_unstall_sucessfully'=>'卸載插件 ( {name} ) 成功，要徹底刪除插件，請手工刪除 {dir} 目錄',
	'plugin_enable_sucessfully'=>'啟用插件 ( {name} ) 成功',
	'plugin_disable_sucessfully'=>'禁用插件 ( {name} ) 成功',
	'plugin_upgrade_sucessfully'=>'升級插件 ( {name} ) 成功',
	'plugin_not_need_update'=>'已經是最新版本，無需更新',
	'plugin_set_relatied_dir_writable'=>'在安裝插件目錄期間，請設置：{dir} 和文件為可寫',
	'plugin_dependency_following'=>'依賴以下插件：{s}，請先安裝依賴的插件',
	'plugin_being_dependent_cant_delete'=>'不能刪除{name}，以下插件依賴它：{s}，',
	'server_response_empty'=>'服務器返回數據為空',
	'server_response_error'=>'服務器返回數據有錯',
	'zip_data_error'=>'壓縮包數據有誤',
	'format_maybe_error'=>'格式可能不正確',
	'plugin_maybe_download_failed'=>'插件可能下載失敗，目錄不存在:',
	'plugin_name_error'=>'插件名不合法',
	'plugin_unstall_confirm_tips'=>'卸載會清理該插件相關數據，確定卸載 ( {name} ) 嗎？',
	'plugin_unstall_batch_confirm_tips'=>'確定要批量卸載所選插件嗎？此操作不可恢復！',
	'plugin_install_batch_sucessfully'=>'所選插件安裝完成',
	'plugin_unstall_batch_sucessfully'=>'所選插件卸載完成',
	'plugin_task_locked'=>'另外壹個插件任務正在執行，當前任務被鎖住。',
	'plugin_return_data_error'=>'返回數據有誤：',
	'plugin_is_free'=>'該插件免費。',
	'plugin_is_not_free'=>'該插件需要付費購買，請先支付。',
	'plugin_is_bought'=>'已經購買過。',
	'plugin_not_bought'=>'還沒購買過。',
	'plugin_wechat_qrcode_pay'=>'微信掃碼支付。',
	'plugin_service_qq'=>'客服 QQ',
	
	// hook lang_zh_tw_bbs_admin.php
	
);

?>
