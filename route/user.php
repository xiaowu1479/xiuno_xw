...
if(empty($action)) {
	...
	if($method == 'GET') {
		...
		$check = user_password_verify($password, $_user);
...
// 重设密码第 3 步 | reset password step 3
	...
	if($method == 'GET') {
		...
		user_update($_uid, array('password'=>user_password_hash($password)));
...
// 发送验证码
	...
	if(empty($smtplist) || !is_array($smtplist)) {
		xn_log('SMTP 配置为空，无法发送邮件', 'send_mail_error');
		message(-1, lang('smtp_not_configured'));
	}
