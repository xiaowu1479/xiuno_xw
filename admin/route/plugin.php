...
// 下载官方插件。 / download official plugin
	...
	// 卸载插件
	FALSE === plugin_unstall($dir) AND message(-1, lang('plugin_unstall_failed', array('dir'=>"plugin/$dir")));
...
} elseif($action == 'install_batch') {
	
	$dirs = param('dirs', array());
	empty($dirs) AND message(-1, lang('please_select_plugin'));
	...
	message(0, jump(lang('plugin_install_batch_sucessfully'), url("plugin-local"), 3));
	
} elseif($action == 'unstall_batch') {
	...
	message(0, jump(lang('plugin_unstall_batch_sucessfully'), url("plugin-local"), 3));
...
function plugin_download_unzip($dir) {
	...
	!is_dir(APP_PATH."plugin/$dir") AND message(-1, lang('plugin_maybe_download_failed')." plugin/$dir");
}
