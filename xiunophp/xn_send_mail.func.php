  ...
  */
  ...
  protected function EncodeFile($path, $encoding = 'base64') {
    try {
      ...
      // PHP 8.0+ 已移除 magic_quotes，兼容旧版本
	  $magic_quotes = false;
	  ...
	  if (version_compare(PHP_VERSION, '5.4', '<') && $magic_quotes) {
...
function xn_send_mail($smtp, $username, $email, $subject, $message, $charset = 'UTF-8') {
	...
	if(empty($smtp['host']) || empty($smtp['user']) || empty($smtp['pass'])) {
		return xn_error(-1, 'SMTP 配置不完整，请检查 host/user/pass 配置项');
	}
	if(empty($email)) {
		return xn_error(-1, '收件人邮箱地址不能为空');
	}
	...
	$mail->Port       = intval($smtp['port']); // set the SMTP port
	
	// 自动检测加密方式：端口 465 默认 SSL，端口 587 默认 TLS
	if(!empty($smtp['secure'])) {
		$mail->SMTPSecure = $smtp['secure']; // "ssl" 或 "tls"
	...
	$mail->Timeout    = isset($smtp['timeout']) ? intval($smtp['timeout']) : 10;
