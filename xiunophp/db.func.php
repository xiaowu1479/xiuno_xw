...
// 读写分离路由 — 不改变现有接口签名
function db_conn_read($d = NULL) {
	if ($d !== NULL) return $d;
	$db = $_SERVER['db'];
	if (!$db) return NULL;
	$conf = $_SERVER['conf'];
	if (!empty($conf['db']['pdo_mysql']['slaves'])) {
		$slaves = $conf['db']['pdo_mysql']['slaves'];
		$idx = array_rand($slaves);
		$slaveconf = $conf['db'];
		$slaveconf['pdo_mysql']['master'] = $slaves[$idx];
		static $slave_conns = array();
		$key = $idx;
		if (!isset($slave_conns[$key])) {
			$slave_conns[$key] = db_new($slaveconf);
		}
		return $slave_conns[$key];
	}
	return $db;
}

function db_conn_write($d = NULL) {
	if ($d !== NULL) return $d;
	return $_SERVER['db'];
}

// 兼容旧版 db_read($table, $cond) — 单行查询
