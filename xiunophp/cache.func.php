...
function cache_get($k, $c = NULL) {
	...
	try {
		...
		xn_log('cache_get failed: ' . $e->getMessage() . ' key: ' . $k, 'cache_error');
		return NULL;
	}
}
...
function cache_set($k, $v, $life = 0, $c = NULL) {
	...
	try {
		...
		xn_log('cache_set failed: ' . $e->getMessage() . ' key: ' . $k, 'cache_error');
		return FALSE;
	}
}
...
function cache_delete($k, $c = NULL) {
	...
	try {
		...
		xn_log('cache_delete failed: ' . $e->getMessage() . ' key: ' . $k, 'cache_error');
		return FALSE;
	}
}
